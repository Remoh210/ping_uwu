...
teletips=Client(
    "PingAllBot",
    api_id = 792253,
    api_hash = "e919e769f4f20b86715742a0a1291a5d",
    bot_token = "6480687583:AAGK-tvh8Ajpg4CgOvOSiz1QFT61HqIqDKI"
)
...
@teletips.on_message(filters.command(["print","list"]))
async def ignore(client, message):
  # Open the file for reading
  with open(str(message.chat.id) + "_ignore.txt", "r") as file:
    content = file.read()


  output_string = "Ignored users: {list}".format(list = content)
  await message.reply(output_string[:-1])

@teletips.on_message(filters.command(["clear","clearignore"]))
async def ignore(client, message):
  os.remove(str(message.chat.id) + "_ignore.txt")
  await message.reply("The owd ignowe wist has been cweawed! ♡(´∩｡• ᵕ •｡∩`)♡")

@teletips.on_message(filters.command(["addingore","ignore"]))
async def ignore(client, message):
  global stopProcess
  ...
  async for member in teletips.get_chat_members(message.chat.id):
    if member.user.is_bot == True:
      ...
      membersList.append(member.user.username)


  nicknames = message.text.split()
  for nickname in nicknames:
    if nickname in membersList:
    # if nickname.startswith("@"):
      ignore_list.append(nickname)

  ignore_list_string = ",".join(ignore_list)

  with open(str(message.chat.id) + "_ignore.txt", "a") as file:
      file.write(ignore_list_string + ",")

  print("ignore_list:")
  print(ignore_list)

  print("ignore_list_string:")
  print(ignore_list_string)

  output_message_text = "Adde two usew names to the ignow wist: " + ", ".join(ignore_list) + ". \n(｡♡‿♡｡)\nI will ignowe these little meanie-wanies OwO when you use the /ping or /all command next time nya~ ♡(^ω^)"

  await message.reply(output_message_text)
